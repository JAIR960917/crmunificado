...
export function resolveCompanyForCity(
  cidade: string | null | undefined,
  routes: CidadeLojaRoute[],
): CidadeLojaRoute | null {
  if (!cidade?.trim() || routes.length === 0) return null;
...
export function distributeUsersEqually(
  submissionIds: string[],
  eligibleUserIds: string[],
): Map<string, string> {
  const result = new Map<string, string>();
  if (eligibleUserIds.length === 0 || submissionIds.length === 0) return result;
  const sorted = [...eligibleUserIds].sort();
  submissionIds.forEach((id, idx) => {
    result.set(id, sorted[idx % sorted.length]);
  });
  return result;
}
