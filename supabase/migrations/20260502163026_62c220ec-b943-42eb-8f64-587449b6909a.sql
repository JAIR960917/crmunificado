...
DECLARE
  ...
  _base_url text := COALESCE(
    NULLIF((SELECT setting_value FROM public.system_settings WHERE setting_key = 'backend_public_url' LIMIT 1), ''),
    NULLIF(current_setting('app.settings.supabase_url', true), '')
  );
  _anon_key text := COALESCE(
    NULLIF((SELECT setting_value FROM public.system_settings WHERE setting_key = 'backend_anon_key' LIMIT 1), ''),
    NULLIF(current_setting('app.settings.supabase_anon_key', true), '')
  );
BEGIN
  IF _base_url IS NULL OR _anon_key IS NULL THEN
    RAISE NOTICE 'Configuração ausente: backend_public_url/backend_anon_key ou app.settings.supabase_* não definidos (cron ssotica não será agendado agora)';
    RETURN;
  END IF;

  _base_url := regexp_replace(trim(_base_url), '/+$', '');
  ...
  _job_command := 'SELECT net.http_post(url := ''' || _base_url || '/functions/v1/ssotica-watchdog'', headers := ''{"Content-Type":"application/json","Authorization":"Bearer ' || _anon_key || '"}''::jsonb, body := ''{}''::jsonb);';
  PERFORM cron.schedule('ssotica-watchdog', '*/15 * * * *', _job_command);
END;
$$;
